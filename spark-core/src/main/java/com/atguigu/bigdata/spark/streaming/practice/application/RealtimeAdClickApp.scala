...
import com.atguigu.bigdata.spark.streaming.practice.service.{BlackListHandler, DateAreaCityAdCountHandler}
...
object RealtimeAdClickApp {
  def main(args: Array[String]): Unit = {

    //1.创建 SparkConf
    val sparkConf: SparkConf = new SparkConf().setMaster("local[*]").setAppName("RealtimeAdClickApp")
